...
condition <- grepl("vehicle", SCC[, SCC.Level.Two], ignore.case=TRUE)  #subset of the NEI data of vehicles
vehiclesSCC <- SCC[condition, SCC]
vehiclesNEI <- NEI[NEI[, SCC] %in% vehiclesSCC,]
vehiclesBaltimoreNEI <- vehiclesNEI[fips == "24510",] #Subset the vehicles NEI data by cityflip and city name
vehiclesBaltimoreNEI[, city := c("Baltimore City")]
vehiclesLANEI <- vehiclesNEI[fips == "06037",]
vehiclesLANEI[, city := c("Los Angeles")]

bothNEI <- rbind(vehiclesBaltimoreNEI,vehiclesLANEI) #combine both
